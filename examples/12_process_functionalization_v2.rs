...
const SAVE_PARQUET_DIR:&str = "D:/data/processed/240903_m3_9_displacement/12_process_functionalization_v2/";
...
fn main() -> Result<()>{
    ...
    for (start,end) in rows{
        ...
        let deviation_first_plot_path:String= format!("{}{}_deviation_first.png",SAVE_PARQUET_DIR,datetime_string);
        make_deviation_first_plot_png(&deviation_df, &deviation_first_plot_path, y_axis_max)?;

        let delta_first_plot_path:String= format!("{}{}_delta_first.png",SAVE_PARQUET_DIR,datetime_string);
        make_delta_first_plot_png(&delta_df, &delta_first_plot_path, y_axis_max)?;
...
fn make_deviation_first_plot_png(df:&DataFrame,save_file_path:&str ,y_axis_max:f64)->Result<()>{
    ...
    let std_deviation: f64 = df.column("deviation_mm")?.std(1).unwrap_or(0.0);
    let caption:String = format!("シール毎のエッジの位置 先頭200袋 最大絶対値:{:.3}、3σ：{:.3}", max_abs_deviation,std_deviation * 3.0);
...
fn make_delta_first_plot_png(df:&DataFrame,save_file_path:&str ,y_axis_max:f64)->Result<()>{

    let max_delta:f64= df.column("delta")?.max()?.unwrap_or(0.0);
    let min_delta:f64 = df.column("delta")?.min()?.unwrap_or(0.0);
    let max_abs_delta = max_delta.abs().max(min_delta.abs());
    let std_delta: f64 = df.column("delta")?.std(1).unwrap_or(0.0);
    let caption:String = format!("前回シールとの差 先頭200袋  最大絶対値:{:.3}、3σ：{:.3}", max_abs_delta,std_delta * 3.0);
